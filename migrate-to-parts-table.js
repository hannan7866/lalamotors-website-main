...
async function migrateToPartsTable() {
    try {
        ...
        // Step 5: Optional - Drop old table (uncomment if you want to remove old table)
        /*
        console.log('Dropping old autoparts table...');
        const { error: dropError } = await supabase.rpc('drop_table_if_exists', { table_name: 'autoparts' });
        if (dropError) {
            console.error('Error dropping old table:', dropError);
        } else {
            console.log('✅ Old autoparts table dropped');
        }
        */
